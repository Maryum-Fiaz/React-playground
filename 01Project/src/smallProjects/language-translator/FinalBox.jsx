...
function FinalBox() {
  ...
  const finalSourcePlaceholder =
    from === "en" ? "Type your text here..." : sourcePlaceholderTranslation;
  const finalTargetPlaceholder =
    to === "en" ? "Type your text here..." : targetPlaceholderTranslation;
      ...
      <div className="w-full p-4 max-w-md mx-auto bg-white rounded-lg shadow-lg">
        <InputBox
          ...
          onLangChange={(lang) => {
            if (sentence) {
              fetch(
                `https://api.mymemory.translated.net/get?q=${sentence}&langpair=${from}|${lang}`,
              )
                .then((res) => res.json())
                .then((res) => {
                  if (res.responseData) {
                ...
                .catch((err) =>
                  console.log("Error changing source text language: ", err),
                );
            }
